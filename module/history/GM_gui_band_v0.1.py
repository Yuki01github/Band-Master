...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        self.master.title("Graph Maker v2.1")
    ...
    def create_menu(self):
        ...
        file_menu.add_command(
            label="ソース", command=self.source_file_path, accelerator="Ctrl+S")
        file_menu.add_command(
            label="保存先", command=self.dst_dir_path, accelerator="Ctrl+D")
        file_menu.add_separator()
        file_menu.add_command(
            label="開く", command=self.open_func, accelerator="Ctrl+O")
        file_menu.add_command(
            label="csvの新規作成", command=create_csv_func, accelerator="Ctrl+N")
        file_menu.add_separator()
    ...
    def create_statusbar(self):
        ...
        self.statusbar.grid(
            row=20, column=0, columnspan=self.root_width, pady=13)
    ...
    def arrangement(self):
        ...
        self.chk_gridflag = tk.Checkbutton(
        ...
        self.chk_gtype.grid(row=19, column=8, sticky=tk.W)
    ...
    def image_reading(self, src, dst, format='png'):
        ...
        self.graph = ImageTk.PhotoImage(
            img, width=self.canvas_width, height=self.canvas_height, master=self.master)
    ...
    def gm_func(self):
        ...
        try:
            ...
            g_type = int(self.bln_gtype.get())

            reg_params = GM.graph_making(src, dst, title, legends, colors, ranges,
                                         markers, sizes,  reg_range, lr, legendf, gridf,  g_type, xlabel, ylabel)
