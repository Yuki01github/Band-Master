...
def main():
    ...
    file_path = r"C:\Users\rinko\OneDrive\デスクトップ\KeioUniversity\history/2021_autumn_4Q/物理情報工学実験CD/history/B3_超伝導/graph.csv"
    df = read_clipboard()
    # L = len(df.columns) - 1
    L = len(df.columns) // 2
    ...
    graph_making(src, dst, title, legends, colors, ranges, markers,
                 sizes, reg_range, lr, legendf, gridf, g_type, xlabel, ylabel)
...
def graph_making(src, dst, title,  legends, colors, ranges, markers,
                 sizes, reg_range, lr, legendf=1, gridf=1, g_type=0, xlabel='x',  ylabel='y'):
    minorticksf = 1
    file_path = src
    ...
    if g_type:
        ...
        # yc = (sqrt(2)-1)*x_lightline_center/2+(2-sqrt(2))/2
